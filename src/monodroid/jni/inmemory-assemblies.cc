...
InMemoryAssemblies::load_assembly_from_memory (MonoDomain *domain, MonoAssemblyName *name)
{
	...
	for (int i = 0; i < asm_count; i++) {
		...
		if (strcmp (asm_name, entry_name) == 0) {
			...
			MonoImage *image = monoFunctions.image_open_from_data_with_name ((char*)entry_bytes, entry_bytes_len, /*need_copy:*/true, &status, /*refonly:*/false, entry_name);
			if (!image || status != MonoImageOpenStatus::MONO_IMAGE_OK) {
				if (image)
					monoFunctions.image_close (image);
				log_error (LOG_DEFAULT, "Couldn't load in-memory image for assembly %s", entry_name);
				return nullptr;
			}
			...
			if (mono_assembly) {
				monoFunctions.image_close (image);
				log_info (LOG_DEFAULT, "Skipping %s that was already loaded", entry_name);
				return nullptr;
			}
			...
			log_info (LOG_DEFAULT, "Loaded %s from memory", entry_name);
